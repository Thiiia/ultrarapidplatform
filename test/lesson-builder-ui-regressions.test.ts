...
test("readiness blocker details paginate dense stress lists", () => {
  assert.match(readinessSource, /const READINESS_BLOCKER_PAGE_SIZE = 25/);
  assert.match(
    readinessSource,
    /visibleOtherBlockers\s*=\s*readiness\.blockers\.slice\([\s\S]{0,160}READINESS_BLOCKER_PAGE_SIZE/,
  );
  assert.match(readinessSource, /aria-label="Readiness blocker pages"/);
  assert.match(readinessSource, /otherBlockerPageCount - 1/);
});
