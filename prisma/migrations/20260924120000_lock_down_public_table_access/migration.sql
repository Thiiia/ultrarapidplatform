-- P9.2 only: remove Supabase Data API grants from server-owned application data.
-- This migration deliberately does not enable or force RLS. RLS is a separate
-- P9.3 gate after the deployed Prisma role and its RLS behavior are verified.
--
-- Source audit: the platform uses Auth0 + server-side Prisma for relational
-- data. Supabase JS use is server-side Storage only. SongAsset is listed in a
-- separate block because song listing is a required regression surface.
...
-- SongAsset is revoked separately after the P9.1 code audit confirmed its
-- browser-facing listing path is a server API backed by Prisma, not PostgREST.
REVOKE ALL PRIVILEGES ON TABLE public."SongAsset" FROM anon, authenticated;

-- The only application-owned public function found is a trigger function,
-- not a PostgREST RPC. No browser or server code calls it directly.
REVOKE EXECUTE ON FUNCTION public.enforce_game_content_revision_state_machine()
  FROM PUBLIC, anon, authenticated;

-- Keep future Prisma-created public tables and sequences private for the
-- verified application migration owner rather than whichever role happens to
-- invoke a migration runner.
...
-- schema-specific REVOKE cannot cancel that global default, so remove it for
-- functions created by the verified application migration owner. Existing
-- functions and defaults for other creator roles stay out of scope.
ALTER DEFAULT PRIVILEGES FOR ROLE postgres
  REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC, anon, authenticated;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public
  REVOKE EXECUTE ON FUNCTIONS FROM anon, authenticated;

-- Fail the migration if any named application table remains reachable through
-- inherited/direct grants, or if the current migration role still defaults
-- privileges to anon/authenticated. PostgreSQL 17 adds MAINTAIN to table ACLs.
DO $verify$
DECLARE
  v_table text;
  v_role text;
...
BEGIN
    ...
    FOREACH v_role IN ARRAY ARRAY['anon', 'authenticated'] LOOP
      FOREACH v_privilege IN ARRAY ARRAY[
        ...
        IF has_table_privilege(v_role, format('public.%I', v_table), v_privilege) THEN
          RAISE EXCEPTION 'P9.2 verification failed: role % retains % on public.%',
  ...
  IF has_function_privilege('anon', 'public.enforce_game_content_revision_state_machine()', 'EXECUTE')
    OR has_function_privilege('authenticated', 'public.enforce_game_content_revision_state_machine()', 'EXECUTE') THEN
