...
namespace CrazyGames.WindowComponents.TextureOptimizations
{
    ...
    {
        public TextureTree(BaseTreeViewState treeViewState, MultiColumnHeader multiColumnHeader, TreeModel<TextureTreeItem> model)
        ...
        void SortIfNeeded(BaseTreeViewItem root, IList<BaseTreeViewItem> rows)
        {
            ...
            var items = rootItem.children.Cast<TreeViewDataItem<TextureTreeItem>>().OrderBy(i => i.data.TextureName);
        ...
        {
            var item = (TreeViewDataItem<TextureTreeItem>) args.item;
        ...
        private void CellGUI(Rect cellRect, TreeViewDataItem<TextureTreeItem> item, int column, ref RowGUIArgs args)
        {
