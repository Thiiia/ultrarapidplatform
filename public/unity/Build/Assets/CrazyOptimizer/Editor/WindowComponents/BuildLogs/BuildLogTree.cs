...
namespace CrazyOptimizer.Editor.WindowComponents.BuildLogs
{
    ...
    {
        public BuildLogTree(BaseTreeViewState treeViewState, MultiColumnHeader multiColumnHeader, TreeModel<BuildLogTreeItem> model)
        ...
        void SortIfNeeded(BaseTreeViewItem root, IList<BaseTreeViewItem> rows)
        {
            ...
            var items = rootItem.children.Cast<TreeViewDataItem<BuildLogTreeItem>>().OrderBy(i => i.data.size);
        ...
        {
            var item = (TreeViewDataItem<BuildLogTreeItem>) args.item;
        ...
        private void CellGUI(Rect cellRect, TreeViewDataItem<BuildLogTreeItem> item, int column, ref RowGUIArgs args)
        {
