            ...
            while ((enumerator.MoveNext()) && (enumerator.Current != null))
            {
                ...
                if (!currentLine.Equals(string.Empty))
                    ...
                    else if (!currentLine.Contains("{"))
                    {
                        ...
                        {
                            // Merge same-tick SyncTrack lines (e.g. "TS" + "B" at tick 0) into both:
                            // 1) the rolling previous sync accumulator and
                            // 2) the already-added list entry for this tick.
                            //
                            // Without this, charts that declare TS and BPM on the same tick can keep the
                            // stored sync entry at BPM=0, which collapses note.Seconds to 0 and breaks
                            // beat windows / end-of-song timing (observed with Grafix - Waves.chart).
                            previousSynchTrack.ProcessLine(currentLine);

                            if (synchTracksList.Count > 0 && synchTracksList[synchTracksList.Count - 1].Tick == synchTrack.Tick)
                            {
                                synchTracksList[synchTracksList.Count - 1].ProcessLine(currentLine);
                            }
                        }
                        else
                        {
