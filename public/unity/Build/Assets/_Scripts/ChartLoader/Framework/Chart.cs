    ...
    namespace ChartLoader.NET.Framework
    {
        ...
        public class Chart
        {
            ...
            {
                // Normalize and clean up weird characters
                line = line.Replace("\t", "")
                        .Replace("\r", "")
                        .Replace("\n", "")
                ...
                string tag = data[0].Trim().Replace(" ", ""); // key fix here
                string attribute = data.Length > 1 ? data[1].Replace("\"", "").Trim() : "";

                try
                {
                    switch (tag)
                    {
                        case "Name": _chartName = attribute; break;
                        case "Artist": _chartArtist = attribute; break;
                        case "Charter": _chartCharter = attribute; break;
                        case "Offset": _offset = double.Parse(attribute, System.Globalization.CultureInfo.InvariantCulture); break;
                        case "Resolution": _resolution = long.Parse(attribute); break;
                        case "Player2": _player2 = attribute; break;
                        case "Difficulty": _difficulty = int.Parse(attribute); break;
                        case "PreviewStart": _previewStart = double.Parse(attribute, System.Globalization.CultureInfo.InvariantCulture); break;
                        case "PreviewEnd": _previewEnd = double.Parse(attribute, System.Globalization.CultureInfo.InvariantCulture); break;
                        case "Genre": _genre = attribute; break;
                        case "MediaType": _mediaType = attribute; break;
                        case "MusicStream": _musicStream = attribute; break;
                        case "GuitarStream": _guitarStream = attribute; break;
                        case "BassStream": _bassStream = attribute; break;
                        default:
                            Debug.LogWarning($"?? Unknown Song tag: '{tag}' = '{attribute}'");
                            break;
                    }
                }
                catch (Exception ex)
                {
                    Debug.LogError($"[Error] Failed to parse tag '{tag}' with value '{attribute}' ? {ex.Message}");
                }
            }
