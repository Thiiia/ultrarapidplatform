...
namespace ChartLoader.NET.Utils
{
    ...
    public class ChartReader
    {
        ...
        public Chart ParseChartText(string chartText)
        {
            ...
            {
                Debug.LogError("[Error] ChartReader received EMPTY chart data!");
                return null;
            }

            // [OK] Normalize line endings for WebGL
        ...
        private void ProcessLine(string line)
        {
            ...
            // Normalize header lines so we don’t miss [Song] on WebGL due to BOM/whitespace/case
            line = line.Replace("\uFEFF", string.Empty) // BOM
                       .Trim();

            // Case-insensitive header handling
            if (line.Equals("[Song]", StringComparison.OrdinalIgnoreCase))
            {
                _chart.ProcessEnumerator(_fileScanner);
                return;
            }

            if (line.Equals("[SyncTrack]", StringComparison.OrdinalIgnoreCase))
            {
            ...
            // Any other bracketed header is a notes section ([ExpertSingle], etc.)
            ...
            // Non-header line: ignore here (handled inside the active block processors)
        }
        ...
        {
            // Normalize the key so dictionary lookups match exactly later
            NoteType = (NoteType ?? string.Empty)
            ...
            )
        {
            ...
            if (string.IsNullOrEmpty(eventLine.Type))
            {
                // Metadata or non-note line; skip further processing.
                currentEvent = null;
                return;
            }
