        ...
        private bool ShaderHasError(UnityEngine.Object obj)
        {
            switch (obj)
            {
                ...
                case ComputeShader computeShader:
                    return ShaderUtil.GetComputeShaderMessageCount(computeShader) > 0;
                default:
                    // Avoid compile-time dependency on RayTracingShader which may not exist in all editor configurations.
                    var rtShaderType = Type.GetType("UnityEngine.Rendering.RayTracingShader");
                    if (rtShaderType != null && rtShaderType.IsInstanceOfType(obj))
                    {
                        ...
                        if (method != null)
                        {
                            var count = (int)method.Invoke(null, new object[] { obj });
                            return count > 0;
                        }
                    }
                    return false;
